...
appears as an approval card. **The agent's `rollback_deployment` call is now suspended,
waiting.** A human clicks APPROVE, the parked call resumes, the rollback executes, and
...
**Approval is a DOM interaction inside a tool call.** `rollback_deployment` calls
...
| `propose_remediation` | write | Renders an approval card. Does not execute. |
| `rollback_deployment` | **act** | Blocks on human approval; verifies args match the plan |
...
`rollback_deployment` returns `status: "awaiting_approval"` with instructions to call
again — the plan stays live and the retry resolves instantly once approved. The agent is
never left hanging on an unbounded wait.
 ...
 Status  Health  Deploys     Logs        Dependencies
   └───────┴───────┴───────────┴──────────────┘
        ...
        record_finding ──> propose_remediation
               │
        HUMAN APPROVAL  ← a DOM click resolves the parked promise
               │
        rollback_deployment ──> verify_recovery
```
